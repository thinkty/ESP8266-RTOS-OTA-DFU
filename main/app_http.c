...
"<script>function submit() { var file = document.getElementById('input-file').files[0]; var xhr = new XMLHttpRequest();xhr.open('POST', '/');xhr.setRequestHeader('X-Requested-With', 'XMLHttpRequest');xhr.setRequestHeader('X-File-Name', file.name);xhr.setRequestHeader('Content-Type', file.type || 'application/octet-stream');xhr.send(file); }</script>"
"</head>"
"<body>"
...
"<!DOCTYPE html><html lang='en'><head><meta charset='UTF-8'><meta name='viewport' content='width=device-width, initial-scale=1.0'><meta http-equiv='X-UA-Compatible' content='ie=edge'><title>OTA Update</title>"
"<style>body {background-color: #c4c4c4; font-family: Arial, Helvetica, sans-serif; margin: 0; padding: 0; height: 100vh; display: flex; flex-direction: column; justify-content: center; align-items: center; } h1 { padding: 1em 1em 0 1em; margin: 0; } h3 { margin: 0 0 2em 0; } svg { width: 10em; height: auto; } form { display: flex; flex-direction: column; justify-content: center; align-items: center; gap: 1em; } input { cursor: pointer; font-size: large; } </style>"
"</head><body>"
"<h1>Success</h1>"
"</body></html>";

static const char * FAIL_PAGE =
"<!DOCTYPE html><html lang='en'><head><meta charset='UTF-8'><meta name='viewport' content='width=device-width, initial-scale=1.0'><meta http-equiv='X-UA-Compatible' content='ie=edge'><title>OTA Update</title>"
"<style>body {background-color: #c4c4c4; font-family: Arial, Helvetica, sans-serif; margin: 0; padding: 0; height: 100vh; display: flex; flex-direction: column; justify-content: center; align-items: center; } h1 { padding: 1em 1em 0 1em; margin: 0; } h3 { margin: 0 0 2em 0; } svg { width: 10em; height: auto; } form { display: flex; flex-direction: column; justify-content: center; align-items: center; gap: 1em; } input { cursor: pointer; font-size: large; } </style>"
"</head><body>"
"<h1>Failed</h1>"
...
esp_err_t post_handler(httpd_req_t * req)
{
    ...
    if (req->method != HTTP_POST) {
        ...
        httpd_resp_send(req, FAIL_PAGE, strlen(FAIL_PAGE));
        return ESP_OK;
    }
